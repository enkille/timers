...
_common.msg = function ( s )
  local txt = '\31\200[\31\05' .. addon.name .. '\31\200]\31\130 ' .. s;
  --print(txt);
  ...
  if ( duration ~= nil ) then
    if type(duration) == "number" then
      ...
      if ( rx ~= nil ) then
        ...
        local h = ashita.regex.match( duration, '(\\d+)[H|h]{1}' )
        local m = ashita.regex.match( duration, '(\\d+)[M|m]{1}' )
        local s = ashita.regex.match( duration, '(\\d+)[S|s]{1}' )
        if ( h == nil and m == nil and s == nil ) then
          return nil
        else
          if ( h ~= nil ) then
            seconds = seconds + (tonumber(h[2]) * 60 * 60)
          end
          if ( m ~= nil ) then
            seconds = seconds + (tonumber(m[2]) * 60)
          end
          if ( s ~= nil ) then
            seconds = seconds + tonumber(s[2])
          end
        end
      end
    end
