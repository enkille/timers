...
local path = AshitaCore:GetAshitaInstallPath() .. '/config/timers/presets'
...
_presets.create = function(name)
	...
	local data = ashita.settings.JSON:encode_pretty(default_timers, nil, { pretty = true, align_keys = false, indent = '    ' })
	if name ~= nil then
...
_presets.load = function(name)
	local data = ashita.settings.load(path .. '/' .. name .. '.json')
	local timers = { }
	if (data ~= nil) then
...
_presets.get_profiles = function()
	local profiles = { }
	if ashita.file.dir_exists(path) then
		local files = ashita.file.get_dir(path, '*.json', false)
		if ( #files > 0 ) then
			for k,v in ipairs(files) do
				local rx = ashita.regex.match( tostring(v), '(.*)\\.json' )
				table.insert (profiles, rx[1])
			end
		end
	end
